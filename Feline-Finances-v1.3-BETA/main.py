...
def update_render_metrics(new_width, new_height):
    global window_width, window_height, scale_factor
    global render_width, render_height, render_offset_x, render_offset_y

    window_width = max(1, new_width)
    window_height = max(1, new_height)
    scale_factor = min(window_width / WIDTH, window_height / HEIGHT)

    render_width = max(1, int(WIDTH * scale_factor))
    render_height = max(1, int(HEIGHT * scale_factor))
    render_offset_x = (window_width - render_width) // 2
    render_offset_y = (window_height - render_height) // 2

def window_to_game_pos(window_pos):
    x = (window_pos[0] - render_offset_x) / scale_factor
    y = (window_pos[1] - render_offset_y) / scale_factor
    if 0 <= x <= WIDTH and 0 <= y <= HEIGHT:
        return x, y
    return None

def get_game_mouse_pos():
    game_pos = window_to_game_pos(pygame.mouse.get_pos())
    if game_pos is None:
        return -9999, -9999
    return game_pos
...
def present_frame():
    ...
    if render_width == WIDTH and render_height == HEIGHT:
        ...
        scaled_surface = pygame.transform.smoothscale(screen, (render_width, render_height))
        window.blit(scaled_surface, (render_offset_x, render_offset_y))
    pygame.display.flip()

update_render_metrics(window_width, window_height)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.VIDEORESIZE:
                ...
                update_render_metrics(event.w, event.h)

            if event.type == pygame.WINDOWSIZECHANGED:
                update_render_metrics(event.x, event.y)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            game_click_pos = window_to_game_pos(event.pos)
            if game_click_pos is None:
                continue
            ...
            if in_store:
                ...
                if back_rect.collidepoint(game_click_pos):
                    random.choice(click_sounds).play()
                    in_store = False
                else:
                    item_x = overlay_x + 40
                    ...
                    for item, price in store_prices.items():
                        ...
                        if rect.collidepoint(game_click_pos):
                            if money >= price:
                                random.choice(click_sounds).play()
                                money -= price
                                total_spent += price
                ...
                if active_minigame == "trash":
                    ...
                    for trash in trash_items:
                        if not trash["collected"]:
                            ...
                            if trash_rect.collidepoint(game_click_pos):
                                random.choice(click_sounds).play()
                ...
                if feed_btn.collidepoint(game_click_pos) and inventory.get("Meowmunch", 0) > 0:
                    random.choice(click_sounds).play()
                    cat.feed()
                    inventory["Meowmunch"] -= 1
                if play_btn.collidepoint(game_click_pos) and inventory.get("Purrplay", 0) > 0:
                    random.choice(click_sounds).play()
                    cat.play()
                    inventory["Purrplay"] -= 1
                if rest_btn.collidepoint(game_click_pos):
                    random.choice(click_sounds).play()
                    cat.rest()
                if clean_btn.collidepoint(game_click_pos) and inventory.get("Furbath", 0) > 0:
                    random.choice(click_sounds).play()
                    cat.clean()
                    inventory["Furbath"] -= 1
                if chore_btn.collidepoint(game_click_pos):
                    random.choice(click_sounds).play()
                    in_chore = True
                if store_btn.collidepoint(game_click_pos):
                    random.choice(click_sounds).play()
