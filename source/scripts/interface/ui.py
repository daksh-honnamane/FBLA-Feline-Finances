...
from core.config import BLACK, BLUE, BUTTON, BUTTON_HOVER, CAT_TYPES, COZY, HEIGHT, PANEL, PERSONALITIES, WHITE, WIDTH
...
class UIContext:
    ...
    def present_frame(self):
        ...
        if self.overlay_active:
            window_dim_surf = pygame.Surface((self.window_width, self.window_height))
            window_dim_surf.set_alpha(120)
            window_dim_surf.fill(BLACK)
            self.window.blit(window_dim_surf, (0, 0))
...
def setup_screen(ui, click_player, cat_images):
    ...
    while True:
        ...
        cat_image = pygame.transform.scale(cat_images[CAT_TYPES[type_index]], (155, 155))
