...
from core.config import BLACK, BLUE, BUTTON, BUTTON_HOVER, CAT_TYPES, COZY, HEIGHT, PANEL, PERSONALITIES, PERSONALITY_DESCRIPTIONS, WIDTH


class UIContext:
    """Owns rendering surfaces, scaling math, and common UI drawing helpers."""
    ...
    def update_render_metrics(self, new_width, new_height):
        """Recompute letterboxed render dimensions when the window is resized."""
    ...
    def window_to_game_pos(self, window_pos):
        """Map physical window coordinates into fixed game-surface coordinates."""
    ...
    def get_game_mouse_pos(self):
        """Return a safe in-bounds mouse position for hover logic."""
    ...
    def present_frame(self):
        """Composite current frame to the window and apply overlay dimming when needed."""
    ...
    def draw_text(self, text, x, y, color=BLACK, font=None):
        """Draw a single text string to the game surface."""
    ...
    def draw_wrapped_text(self, text, x, y, width, color=BLACK, font=None, line_gap=6):
        """Draw multi-line text constrained to a width and return final y position."""
        active_font = font or self.font
        words = text.split()
        line = ""
        line_y = y
    ...
    def draw_stat_bar(self, x, y, width, height, value, max_value, color):
        """Render a bordered progress bar for stats like health or energy."""
    ...
    def draw_button(self, rect, text, check_hover=True, fill_color=None, text_color=BLACK):
        """Render a rounded button with optional hover highlight and custom color."""
    ...
    def overlay_rect(self):
        """Return the standard centered overlay bounds used by all modal screens."""
    ...
    def draw_overlay_shell(self, title, subtitle=""):
        """Draw a reusable modal container and return its rect plus back button rect."""
...
def setup_screen(ui, click_player, cat_images):
    """Run the character creation screen and return chosen cat attributes."""
    name = ""
    type_index = 0
    personality_index = 0
    name_active = True
    ...
    while True:
        # Build panel geometry every frame so hitboxes always match current UI layout.
        ui.screen.fill(COZY)
        ...
        ui.draw_wrapped_text(
            PERSONALITY_DESCRIPTIONS[PERSONALITIES[personality_index]],
            info_rect.x + 18,
            info_rect.y + 16,
            info_rect.width - 36,
            font=ui.small_font,
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # Each branch updates exactly one piece of setup state.
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN and name:
                    ...
                    return name, CAT_TYPES[type_index], PERSONALITIES[personality_index]
                if name_active:
                    # Restrict names to short alphabetic input for clean save data.
