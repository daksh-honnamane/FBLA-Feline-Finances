...
class Game:
    """Top-level game controller handling state, input, updates, and rendering."""
    ...
    def __init__(self):
        ...
        self.vet_btn = pygame.Rect(490, 500, 100, 40)
        self.menu_btn = pygame.Rect(700, 350, 160, 40)
        self.inventory_btn = pygame.Rect(700, 400, 160, 40)
        ...
        self.oven_safe_min = 0.42
        self.oven_safe_max = 0.58
    ...
    def clear_overlays(self):
        """Close every overlay and return to main gameplay view."""
        self.overlay_stack.clear()
        self.active_overlay = None
        self.reset_chore_state()

    def get_chore_catalog(self):
        """Return display metadata for all chore minigames."""
        return [
            ("Take Out The Trash", "trash", "$10"),
            ("Put Away Laundry", "laundry", "$15"),
              ("The Purrfect Bake", "oven", "$20"),
        ]

    def get_oven_layout(self, rect):
        """Compute gauge and hold-button bounds inside the oven chore panel."""
    ...
    def setup_oven_round(self, state):
        """Initialize per-round timers and pointer values for the oven chore."""
        if "pointer" not in state:
            state["pointer"] = 0.5
        state["round_timer"] = self.oven_round_duration
        state["undercook_time"] = 0
        state["burn_time"] = 0
        if "hold_active" not in state:
            state["hold_active"] = False

    def finish_oven_round(self, result):
        """Record one oven result and return completion flag plus status message."""
        ...
        if completed_rounds >= 6:
            if success_count >= 4:
                return True, f"The Purrfect Bake finished with {success_count}/6 cakes baked just right. Earned $20."
    ...
    def update_active_chore(self, dt):
        """Advance the active chore simulation and emit completion when done."""
        ...
        # Pointer rises while holding heat and drifts down when released.
    ...
    def handle_overlay_back(self):
        """Handle ESC/back behavior, including returning from active chore screens."""
    ...
    def point_in_rounded_rect(self, point, rect, radius):
        """Hit-test against a rounded rectangle used on the title art button."""
    ...
    def get_title_draw_metrics(self, title_size):
        """Calculate scaled title dimensions and centered drawing offsets."""
    ...
    def window_to_title_pos(self, window_pos, title_size, scale, offset_x, offset_y):
        """Convert window click coordinates into original title-image coordinates."""
    ...
    def show_title_screen(self):
        """Render and process the title screen until start or quit."""
        while self.running:
    ...
    def play_click(self):
        """Play one random click sound for UI feedback."""
        random.choice(self.click_sounds).play()

    def load_or_create_game(self):
        """Load save data when available, otherwise create a new game state."""
        loaded = load_game()
    ...
    def start_new_game(self):
        """Launch setup flow and return fresh cat plus default progress."""
    ...
    def get_mood(self):
        """Derive a high-level mood label from current stat thresholds."""
    ...
    def get_status_color(self):
        """Map the current mood to a UI accent color."""
        mood = self.get_mood()
        if mood == "Critical":
            return RED
        if mood == "Sick":
    ...
    def unlock_achievements(self):
        """Evaluate achievement conditions and queue badge notifications."""
    ...
    def get_badge_catalog(self):
        """Return badge metadata used by the badges overlay."""
        return [
            {
                "name": "Chore Champion",
    ...
    def get_report_lines(self):
        """Build human-readable report lines for the summary overlay."""
        return [
    ...
    def draw_pet_display(self):
        """Draw cat portrait, aura, and mood badge in the center panel."""
    ...
    def handle_store_purchase(self, item_name):
        """Attempt to buy one store item and update budget/inventory state."""
    ...
    def attempt_action(self, action_name):
        """Run a pet-care action, consuming required inventory when needed."""
        if action_name == "feed":
    ...
    def perform_vet_visit(self):
        """Spend money at the vet and restore cat health."""
    ...
    def spawn_chore_state(self, chore_id, rect):
        """Create initial state for the selected chore minigame."""
        if chore_id == "trash":
            items = []
            # Random placement keeps each trash round slightly different.
            for _ in range(5):
    ...
    def handle_chore_click(self, click_pos, rect=None):
        """Process click actions for whichever chore is currently active."""
    ...
    def update_laundry_drag(self, mouse_pos):
        """Move dragged laundry item with the cursor while preserving click offset."""
    ...
    def finish_laundry_drag(self, mouse_pos):
        """Drop dragged laundry piece and detect bin placement completion."""
        if self.active_chore != "laundry" or self.dragged_laundry_index is None:
            return False, ""
    ...
    def reset_chore_state(self):
        """Clear active chore and reset any drag-related temporary state."""
        self.active_chore = None
        self.chore_state = {}
        self.dragged_laundry_index = None
        self.drag_offset = (0, 0)
    ...
    def should_show_overlay_status(self):
        """Return whether status text should appear in overlay panels."""
        return not self.dead_reason and self.status_message not in {
            "",
    ...
    def should_show_store_status(self):
        """Return whether store-specific purchase warnings should be shown."""
        return not self.dead_reason and self.status_message == "Insufficient funds for that purchase."

    def draw_help_overlay(self):
        """Render controls and strategy guidance overlay."""
        ...
        left_lines = [
            ...
            "Play uses Purrplay and raises happiness.",
    ...
    def draw_delete_save_overlay(self):
        """Render confirmation overlay for destructive save reset action."""
    ...
    def draw_report_overlay(self):
        """Render current run snapshot and unlocked achievements."""
    ...
    def draw_inventory_overlay(self):
        """Render inventory panel with item purpose and current stock."""
    ...
    def draw_menu_overlay(self):
        """Render central menu for secondary overlays and destructive actions."""
    ...
    def draw_badges_overlay(self):
        """Render earned and locked badges with visual status treatment."""
    ...
    def draw_vet_overlay(self):
        """Render vet information and visit action prompt."""
    ...
    def draw_store_overlay(self):
        """Render supply store inventory and current money total."""
        rect, _ = self.ui.draw_overlay_shell("Whiskermart", "Buy supplies before your cat runs out of care items.")

        item_y = rect.y + 104
        for item in STORE_ITEMS:
            item_rect = pygame.Rect(rect.x + 34, item_y, rect.width - 68, 88)
            self.ui.draw_button(item_rect, "")
            self.ui.draw_text(f"{item['name']} - ${item['price']}", item_rect.x + 16, item_rect.y + 14)
            self.ui.draw_wrapped_text(item["description"], item_rect.x + 16, item_rect.y + 44, item_rect.width - 32, font=self.ui.small_font, line_gap=2)
            item_y += 100

        if self.should_show_store_status():
            color = RED if "Insufficient" in self.status_message else BLACK
            self.ui.draw_wrapped_text(self.status_message, rect.x + 34, rect.bottom - 52, rect.width // 2, color=color, font=self.ui.small_font)

        money_surf = self.ui.font.render(f"Money: ${self.progress['money']}", True, BLACK)
        money_rect = money_surf.get_rect(bottomright=(rect.right - 34, rect.bottom - 24))
        self.ui.screen.blit(money_surf, money_rect)

    def draw_chore_overlay(self):
        """Render chore picker or currently active chore minigame."""
        ...
        if self.active_chore == "oven":
            ...
            block_y = rect.y + 138
            results_row_rect = pygame.Rect(rect.x + 48, block_y, rect.width - 96, 40)
            block_gap = 10
            block_width = (results_row_rect.width - block_gap * 5) // 6
            for index in range(6):
                block_rect = pygame.Rect(
                    results_row_rect.x + index * (block_width + block_gap),
                    results_row_rect.y,
                    block_width,
                    results_row_rect.height,
                )
                ...
                label_font = self.ui.small_font if len(label) > 4 else self.ui.font
                label_surf = label_font.render(label, True, BLACK)
                label_rect = label_surf.get_rect(center=block_rect.center)
                self.ui.screen.blit(label_surf, label_rect)
            ...
            self.ui.draw_text("Purrfect", safe_rect.x + 12, gauge_rect.y + 20, font=self.ui.small_font)
            ...
            self.ui.draw_text(f"Purrfect bakes: {success_count}/4", hold_rect.right + 20, hold_rect.y + 15, font=self.ui.small_font)
    ...
    def draw_game_over_overlay(self):
        """Render game-over modal with restart/quit actions."""
    ...
    def handle_overlay_click(self, click_pos):
        """Dispatch mouse clicks to the currently visible overlay controls."""
    ...
    def handle_restart_click(self, click_pos):
        """Handle restart and quit buttons shown after game over."""
    ...
    def handle_main_click(self, click_pos):
        """Handle clicks on primary care, menu, and economy buttons."""
    ...
    def process_death(self):
        """Detect newly depleted stats and finalize game-over state once."""
    ...
    def process_time_step(self):
        """Apply fixed-step stat decay and budget warnings based on elapsed time."""
    ...
    def draw_main_ui(self):
        """Draw always-visible HUD elements and action buttons."""
        panel_x = 50
        panel_y = 140
        panel_spacing = 35
        bar_width = 200
        ...
        self.ui.draw_button(self.vet_btn, "Vet", check_hover=hover_enabled)
        ...
        notifications_hidden = self.active_overlay or self.dead_reason

        if not notifications_hidden and self.status_timer > 0 and self.status_message:
        ...
        if not notifications_hidden and self.badge_timer > 0 and self.badge_message:
    ...
    def draw_active_overlay(self):
        """Render whichever overlay is currently active."""
    ...
    def run(self):
        """Run the full game lifecycle from title screen to shutdown."""
        ...
        while self.running:
            ...
            # Save periodically so progress survives unexpected exits.
...
def run():
    """Convenience entry point used by the top-level launcher."""
    Game().run()