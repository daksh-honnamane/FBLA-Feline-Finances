...
class Game:
    def __init__(self):
        ...
        self.title_images = self.assets["title_images"]
        self.title_configs = {
            "small": {
                "image": self.title_images["small"],
                "button_rect": pygame.Rect(534, 82, 364, 116),
                "button_radius": 22,
            },
            "big": {
                "image": self.title_images["big"],
                "button_rect": pygame.Rect(1187, 171, 809, 242),
                "button_radius": 48,
            },
        }
    ...
    def get_title_config(self):
        if self.ui.window_width > WIDTH or self.ui.window_height > HEIGHT:
            return self.title_configs["big"]
        return self.title_configs["small"]
    ...
    def show_title_screen(self):
        while self.running:
            title_config = self.get_title_config()
            title_image = title_config["image"]
            title_size = title_image.get_size()
            scale, offset_x, offset_y, draw_width, draw_height = self.get_title_draw_metrics(title_size)
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    click_pos = self.window_to_title_pos(event.pos, title_size, scale, offset_x, offset_y)
                    if click_pos is not None and self.point_in_rounded_rect(click_pos, title_config["button_rect"], title_config["button_radius"]):
                        self.play_click()
                        return True

            self.ui.window.fill(COZY)
            scaled_title = pygame.transform.smoothscale(title_image, (draw_width, draw_height))
            self.ui.window.blit(scaled_title, (offset_x, offset_y))
            pygame.display.flip()
    ...
    def should_show_store_status(self):
        return self.status_message.startswith("Bought ") or self.status_message == "Insufficient funds for that purchase."
    ...
    def draw_badges_overlay(self):
        ...
        badge_font = pygame.font.SysFont("Comic Sans MS", 13)
