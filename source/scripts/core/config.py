import sys
from pathlib import Path


# Resolve important paths once so every module can import shared locations.
CORE_DIR = Path(__file__).resolve().parent
SOURCE_DIR = CORE_DIR.parent.parent

# PyInstaller sets sys.frozen and exposes temporary bundled assets via _MEIPASS.
if getattr(sys, "frozen", False):
    RESOURCE_ROOT = Path(getattr(sys, "_MEIPASS", Path(sys.executable).resolve().parent))
    SAVE_ROOT = Path(sys.executable).resolve().parent
else:
    RESOURCE_ROOT = SOURCE_DIR
    SAVE_ROOT = SOURCE_DIR

ASSET_DIR = RESOURCE_ROOT / "assets"
SAVE_FILE = SAVE_ROOT / "save_data.json"

WIDTH, HEIGHT = 900, 600
CAPTION = "Feline Finances"

# Shared color palette used throughout UI rendering.
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
COZY = (249, 230, 197)
BUTTON = (155, 182, 130)
BUTTON_HOVER = (135, 162, 110)
PANEL = (255, 248, 236)
RED = (212, 95, 95)
GOLD = (216, 180, 78)
BLUE = (122, 180, 224)

AUTOSAVE_INTERVAL = 10000
TIME_STEP_MS = 5000
CAT_TYPES = ["Orange", "Grey", "White", "Calico"]
PERSONALITIES = ["Playful", "Lazy", "Shy", "Energetic"]
STAT_NAMES = ("hunger", "happiness", "energy", "cleanliness", "health")

# Personality modifiers are applied in care actions and passive time decay.
PERSONALITY_EFFECTS = {
    "Playful": {
        "feed_bonus": 0,
        "play_bonus": 6,
        "rest_bonus": -4,
        "clean_bonus": 0,
        "hunger_decay": 0,
        "happiness_decay": 0,
        "energy_decay": 1,
        "cleanliness_decay": 0,
    },
    "Lazy": {
        "feed_bonus": 0,
        "play_bonus": -4,
        "rest_bonus": 8,
        "clean_bonus": 0,
        "hunger_decay": 0,
        "happiness_decay": 1,
        "energy_decay": -1,
        "cleanliness_decay": 0,
    },
    "Shy": {
        "feed_bonus": 0,
        "play_bonus": -2,
        "rest_bonus": 0,
        "clean_bonus": 6,
        "hunger_decay": 0,
        "happiness_decay": -1,
        "energy_decay": 0,
        "cleanliness_decay": 0,
    },
    "Energetic": {
        "feed_bonus": 0,
        "play_bonus": 4,
        "rest_bonus": -2,
        "clean_bonus": 0,
        "hunger_decay": 1,
        "happiness_decay": 0,
        "energy_decay": 1,
        "cleanliness_decay": 0,
    },
}

PERSONALITY_DESCRIPTIONS = {
    "Playful": "Play gives extra happiness, but energy falls a little faster over time.",
    "Lazy": "Rest restores extra energy, but happiness slips faster and play helps less.",
    "Shy": "Happiness drops more slowly, and baths give a bigger cleanliness boost.",
    "Energetic": "Play is stronger, but hunger and energy drain a little faster over time.",
}

STORE_ITEMS = [
    {"name": "Meowmunch", "price": 5, "category": "Food", "description": "Crunchy food that restores hunger."},
    {"name": "Purrplay", "price": 10, "category": "Toys", "description": "A toy set that boosts happiness."},
    {"name": "Furbath", "price": 15, "category": "Grooming", "description": "Bath supplies for cleanliness."},
]
ITEM_DETAILS = {item["name"]: item for item in STORE_ITEMS}

VET_COST = 25
VET_HEAL = 35
SAVE_GOAL = 100


def asset_path(*parts: str) -> str:
    """Build an absolute asset path from nested path parts."""
    return str(ASSET_DIR.joinpath(*parts))