...
def create_default_progress():
    """Return baseline progress fields used for new saves and repair paths."""
    return {
        "money": 50,
        "total_spent": 0,
        "total_earned": 0,
...
def sanitize_inventory(raw_inventory):
    """Normalize inventory values from raw save data into safe non-negative ints."""
    inventory = {item["name"]: 0 for item in STORE_ITEMS}
...
def sanitize_expenses(raw_expenses):
    """Normalize expense totals from raw save data into safe non-negative ints."""
...
def sanitize_save_data(raw_data):
    """Validate and convert loaded JSON into a Cat instance plus progress dict."""
    ...
    # Clamp all mutable stats so malformed saves cannot create invalid ranges.
...
def save_game(cat, progress):
    """Write the current game state to disk as formatted JSON."""
    data = cat.as_dict()
...
def load_game():
    """Load save data from disk, returning sanitized state or None when invalid."""
...
def reset_save_file():
    """Delete the save file when starting a clean run."""
